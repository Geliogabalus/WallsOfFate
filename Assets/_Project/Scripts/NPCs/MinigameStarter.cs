...
namespace Game
{
    ...
    public class MinigameStarter : MonoBehaviour
    {
        ...
        {// Обновляем ресурсы в зависимости от результата
            ...
            {
                Player.Resources.ChangePeopleSatisfaction(-1000); // Уменьшаем PeopleSatisfaction
            ...
            {
                Player.Resources.ChangeCastleStrength(10); // Увеличиваем CastleStrength
            }
            else if (winnerName != "Player" && DialogueManager.GetInstance().PowerCheckPrefab != _trainingPrefab)
            {
                Player.Resources.ChangeCastleStrength(-1); // Уменьшаем PeopleSatisfaction
            }
            if (resultPanel != null)
            {
